...
module('Acceptance | Action handling', function(hooks) {
  ...
  test('onFieldValueChange', async function(assert) {
    ...
    await fillIn(`${dummyEls.onFieldValueChangeForm} ${dummyEls.firstNameField} input`, 'G');
    assert.dom(dummyEls.onFieldValueChangeFeedback).hasText(`The user's full name is "G ". The last updated field was First name. The value of settings.formName is names.`, 'All arguments are correctly sent with the onFieldValueChange action.');
    await fillIn(`${dummyEls.onFieldValueChangeForm} ${dummyEls.firstNameField} input`, 'Gene');
    assert.dom(dummyEls.onFieldValueChangeFeedback).hasText(`The user's full name is "Gene ". The last updated field was First name. The value of settings.formName is names.`, 'onFieldValueChange runs when user types in an input field.');
  });
