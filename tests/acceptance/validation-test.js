...
module('Acceptance | Field validation', function(hooks) {
  ...
  test('Validation events', async function(assert) {
    ...
    assert.dom(`${els.nameField} ${els.fireFormFieldErrors}`).doesNotExist('Required but empty field with validation events "keyUp" and "insert" does not have validation error messages on insert.');
    assert.notOk(find(`${els.nameField}`).classList.contains('invalid'), 'Required but empty field with validation events "keyUp" and "insert" does not have class "invalid" on insert.');
    assert.notOk(find(`${els.nameField}`).classList.contains('valid'), 'Required but empty field with validation events "keyUp" and "insert" does not have class "valid" on insert.');
    assert.ok(find(`${els.emailField}`).classList.contains('valid'), 'Valid field with validation event "insert" has class "valid" on insert.');
    assert.dom(`${els.recoveryEmailField} ${els.fireFormFieldErrors}`).exists('Invalid field with validation event "insert" has validation error messages on insert.');
    assert.ok(find(`${els.recoveryEmailField}`).classList.contains('invalid'), 'Invalid field with validation event "insert" has class "invalid" on insert.');
  ...
  test('Validation messages', async function(assert) {
    ...
    assert.equal(findAll(els.fireFormFieldErrors).length, 6, 'All fields with validation rules are validated when user clicks submit button.');
