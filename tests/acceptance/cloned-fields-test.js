...
module('Acceptance | Cloned fields', function (hooks) {
  ...
  test('Basics', async function (assert) {
    ...
    assert.dom(`${dummyEls.clonableFieldBasics} ${els.cwfMaxClonesReached}`).hasText('Max clones reached.', 'Max clones reached text displays when the number of clones is equal to the value of the maxClones setting.');
    assert.dom(`${dummyEls.clonableFieldBasics} ${els.cwfAddClone}`).doesNotExist('Add clone button disappears when maxClones is reached.');
    await fillIn(
      `${dummyEls.clonableFieldBasics} ${els.cloneSelector({
        fieldId: 'emails',
        cloneId: 1,
    ...
    assert.equal(findAll(els.cwfCloneWrapper)[0].getAttribute('data-test-id'), 'emails-clone-1-wrapper', 'Second clone correctly becomes first clone, after first clone is removed.');
    assert.equal(findAll(els.cwfCloneWrapper)[1].getAttribute('data-test-id'), 'emails-clone-2-wrapper', 'Third clone correctly becomes second clone, after first clone is removed.');
  });

  test('With data', async function (assert) {
    await visit('/docs/clonable-form-fields');
    assert.dom(`${dummyEls.clonableFieldWithData} ${els.cwfCloneWrapper}`).exists({ count: 6 }, 'Where number of items in the data array exceeds max clones, one cloned field is still generated for each item in the data array.');
    assert.ok(cth.allFailedValidation(`${dummyEls.clonableFieldWithData} ${els.cwfCloneWrapper}`, [0, 1]), '1) Invalid clones fail validation on insert, where [insert] is a clone validation method. 2) The uniqueClone validation method works.');
    assert.notOk(
      cth.wasValidated(
    ...
    assert.ok(cth.allPassedValidation(`${dummyEls.clonableFieldWithData} ${els.cwfCloneWrapper}`, [3, 4, 5]), 'Valid clones pass validation on insert, where [insert] is a clone validation method');
    assert.dom(els.cwfMaxClonesReached).exists('Max clones reached text shows on insert.');
    ...
    assert.dom(els.cwfMaxClonesReached).exists('Max clones reached text persists after multiple clone removals, where the existing number of clones is equal to the max allowed.');
    await cth.removeClone(dummyEls.clonableFieldWithData);
    assert.dom(`${dummyEls.clonableFieldWithData} ${els.cwfAddClone}`).exists('Add clone button shows after clone removal, where the existing number of clones is less than to the max allowed.');
    assert.dom(`${dummyEls.clonableFieldWithData} ${els.cwfMaxClonesReached}`).doesNotExist('Max clones reached text disappears after clone removal, where the existing number of clones is less than to the max allowed.');
  });

  test('Other', async function (assert) {
    await visit('/docs/clonable-form-fields');
    assert.dom(`${dummyEls.clonableFieldCountries} ${els.cwfCloneWrapper}`).exists({ count: 2 }, '2 clones exist on load.');
