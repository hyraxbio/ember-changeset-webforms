...
const submitActionSyncSuccess = (data) => { return true; };
const submitActionSyncFail = (data) => { throw 'Error'; };
...
module('Integration | Component | changeset-webform', function(hooks) {
  ...
  test('Submit without clearing', async function(assert) {
    ...
    `);
    assert.equal(find(`${dummyEls.nameField} input`).value, '', 'Name field empty on insert.');
    assert.equal(find(`${dummyEls.emailField} input`).value, 'test@email.com', 'Email field has default value "test@email.com" on insert.');
  ...
  test('Submit with clearing, retaining field.defaultValue', async function(assert) {
    this.set('submitAction', submitActionSyncSuccess);
    this.clearWithDefaultsFormSchema = _merge({}, formSchema, {formSettings: {
    ...
    assert.equal(find(`${dummyEls.emailField} input`).value, 'test2@email.com', 'Email field has updated value "test2@email.com" after editing.');
    await click(dummyEls.cwfSubmitFormButton);
    assert.equal(find(`${dummyEls.nameField} input`).value, '', 'Name field is cleared after successful form submission.');
    assert.equal(find(`${dummyEls.emailField} input`).value, 'test@email.com', 'Email field is reset to default "test@email.com" after successful form submission.');
  });
  test('Submit with clearing, suppressing field.defaultValue', async function(assert) {
    this.set('submitAction', submitActionSyncSuccess);
    this.clearWithoutDefaultsFormSchema = _merge({}, formSchema, {formSettings: {
    ...
    await click(dummyEls.cwfSubmitFormButton);
    assert.equal(find(`${dummyEls.nameField} input`).value, '', 'Name field is cleared after successful form submission.');
    assert.equal(find(`${dummyEls.emailField} input`).value, '', 'Email field is cleared after successful form submission.');
