...
module('Integration | Component | validating-form', function(hooks) {
  ...
  test('Initial rendering', async function(assert) {
    ...
    await render(hbs`{{validating-form formSchema=formSchema}}`);
  ...
  test('Basic actions', async function(assert) {
    ...
    await render(hbs`{{validating-form
  ...
  test('Form validation on submit', async function(assert) {
    ...
    let session = this.owner.lookup('service:session');
    this.set('formSchema', session.signupFormSchema);
    var colorsField = this.get('formSchema.fields').find(field => {
      return field.fieldId === 'personal_details.favourite_colours';
    });
